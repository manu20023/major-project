...
import { adminRoute, managerRoute, userRoute } from "./utils/ConditionalRoutes";
...
function App() {
  ...
  useEffect(() => {
    if (error && error.message === "Unauthenticated user!") {
      generateToken({
        variables: { refreshToken: cahedUser.refreshToken },
      }).then((res) => {
          ...
          <Suspense fallback={<PageLoader />}>
            <Routes>
              ...
              <Route
                ...
                path="/"
                element={
                  managerRoute || userRoute || adminRoute ? (
                    <Home />
                  ) : (
                    <Navigate to="/login" />
                  )
