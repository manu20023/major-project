  ...
  return (
    ...
    >
      ...
      <Text className="small">
        Type:{" "}
        {user.isManager ? "Hotel Manager" : user.isAdmin ? "Admin" : "User"}
      </Text>
      {!user.isManager ? (
        <FormButton
          style={{ margin: "16px 0", borderRadius: "2px" }}
          onClick={() =>
            setOpen({
              state: true,
              title: "Add Property",
      ...
      {showBtn ? (
        <FormButton style={{ marginLeft: "auto" }} onClick={updateUser}>
          Update & Save
        </FormButton>
      ) : null}
    </PageContainer>
