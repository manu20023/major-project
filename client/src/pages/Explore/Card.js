...
import {
  ...
  Image,
  Text,
} from "../../components/GlobalStyles/PageStyles";
import "./card.css";
...
const Card = (props) => {
  ...
  const getAvlRoomCount = () => {
    ...
    let roomsCount = hotel.rooms
      .map((room) => room.roomNumbers.length)
      .reduce((cur, res) => cur + res);
    return roomsCount;
  };
    ...
    <CardContainer>
      ...
      <HotelDetails className="details">
        ...
        <Text className="small" style={{ marginBottom: 0 }}>
          {getAvlRoomCount() > 0
            ? `${getAvlRoomCount()} Rooms Available`
            : "No room is available right now"}
        </Text>
        <FormButton
          style={{
            width: "100%",
