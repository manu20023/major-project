...
import {
  ...
  ModalContainer,
  ModalTitle,
} from "../../components/GlobalStyles/ModalStyles";
...
const Dashboard = () => {
    ...
    },
    {
      ...
      icon: (
        <img
          ...
          alt=""
          title=""
          style={{ width: 24, backgroundColor: "white" }}
        ></img>
      ),
      action: () => {
        setQrCodeModal({
  ...
  return (
    <PageContainer>
      {roomModal.state && (
        <RoomModal
          action={roomModal.action}
          title={roomModal.title}
          hotel={hotel}
          room={roomModal.param}
      ...
      {hotelModal.state && (
        <HotelModal
          action={hotelModal.action}
          title={hotelModal.title}
          hotel={hotel}
      ...
      {qrCodeModal.state && (
        <ModalContainer>
          <ModalBox className="modal-box-qrcode">
            <CloseIcon
              className="close-icon"
              onClick={() => setQrCodeModal({ state: false, title: "" })}
            />
            <ModalTitle>{qrCodeModal.title}</ModalTitle>
            <QRCode value={"http://127.0.0.1:3000/hotel/" + data.getHotel.id} />
          </ModalBox>
        </ModalContainer>
      )}
      ...
      <QuickView>
        <Info
          style={{
            backgroundImage: `url(${hotel.image ? hotel.image : HotelIMG})`,
          }}
        >
