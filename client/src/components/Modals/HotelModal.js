...
import {
  ADD_HOTEL,
  UPDATE_HOTEL,
} from "../../graphql/mutations/hotelMutations";
...
const HotelModal = (props) => {
  ...
  const updateCurHotel = async (e) => {
    ...
    if (assignedrooms > hotel.totalRooms) {
      toast.warning(
        `You have already assigned ${assignedrooms} rooms. Unassign those rooms to decrease room numbers.`,
        {
          autoClose: 5500,
          pauseOnHover: true,
        }
        ...
        {!loading ? (
          <>
            <CloseIcon
              ...
              onClick={() => props.setHotelModal(false)}
            />
            ...
            <form
              onSubmit={
                props.action === "update" ? updateCurHotel : addNewHotel
              }
            >
              <Input
                required="true"
